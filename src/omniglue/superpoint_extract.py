...
class SuperPointExtract:
    ...
    def compute(
        ...
        assert (
            segmentation_mask is None or image.shape[:2] == segmentation_mask.shape[:2]
        )
        ...
        output_prob_nms_tensor = self._graph.get_tensor_by_name("superpoint/prob_nms:0")
        output_desc_tensors = self._graph.get_tensor_by_name("superpoint/descriptors:0")
        out = self._sess.run(
